class Account < ApplicationRecord
  ...
  validates_inclusion_of(:role_state, :in => ["customer", "administrator"], :on => :update)
  ...
  state_machine(:role_state, :initial => :customer) do
    event(:upgrade_to_administrator) do
      transition([:customer] => :administrator)
    end

    event(:downgrade_to_user) do
      transition(:from => [:administrator], :to => :customer)
    end
